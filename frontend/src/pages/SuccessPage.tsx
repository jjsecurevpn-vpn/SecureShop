...
const SuccessPage: React.FC = () => {
  ...
  const cargarPago = async (pagoId: string, tipo: string = 'cliente') => {
    try {
      ...
      if (data.estado !== 'aprobado') {
        // Si el pago no está aprobado y no hemos reintentado mucho, reintentar
        if (reintentos < 10) {
          setTimeout(() => {
            console.log('[Success] Reintentando verificación del pago... intento', reintentos + 1);
            setReintentos(prev => prev + 1);
          }, 3000); // Reintentar cada 3 segundos
        } else {
          setError('El pago aún no ha sido procesado. Por favor, verifica tu email o contacta a soporte.');
        }
      }
    } catch (err: any) {
      ...
      if (reintentos < 10) {
        setTimeout(() => {
          console.log('[Success] Reintentando después de error... intento', reintentos + 1);
          setReintentos(prev => prev + 1);
        }, 3000);
      } else {
  ...
  if (loading) {
    return <Loading message={reintentos > 0 ? `Procesando tu pago... (${reintentos}/10)` : "Verificando tu compra..."} />;
  }
