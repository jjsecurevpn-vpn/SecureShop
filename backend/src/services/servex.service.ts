...
export class ServexService {
  ...
  constructor(config: ServexApiConfig) {
    ...
    this.client.interceptors.response.use(
      (response) => {
        console.log(`[Servex] ✅ ${response.config.method?.toUpperCase()} ${response.config.url} - ${response.status}`);
        return response;
      },
      (error) => {
        console.error(`[Servex] ❌ Error: ${error.response?.data?.message || error.message}`);
        throw error;
      }
    );
  }
  ...
  async crearCliente(clienteData: ClienteServex): Promise<ClienteCreado> {
    try {
      ...
      console.log('[Servex] Datos del cliente:', JSON.stringify(clienteData, null, 2));
      const response = await this.client.post<ServexClienteResponse>('/clients', clienteData);
      console.log('[Servex] Respuesta de creación:', JSON.stringify(response.data, null, 2));
      ...
      const detalles = error.response?.data ? JSON.stringify(error.response.data) : '';
  ...
  async obtenerCategorias(): Promise<CategoriaServex[]> {
    try {
      ...
      console.log('[Servex] Categorías obtenidas:', JSON.stringify(response.data, null, 2));
  ...
  async obtenerCategoriasActivas(): Promise<CategoriaServex[]> {
    try {
      ...
      const categoriasActivas = categorias.filter(categoria => {
        ...
        if (!esActiva) {
          console.log(`[Servex] ⚠️ Categoría ${categoria.id} (${categoria.name}) expiró el ${categoria.valid_until}`);
        }
        
        return esActiva;
      });

      console.log(`[Servex] Categorías totales: ${categorias.length}, Activas: ${categoriasActivas.length}`);
      ...
      const mensaje = error.response?.data?.message || error.message;
      throw new Error(`Error obteniendo categorías activas de Servex: ${mensaje}`);
    }
  }
   ...
   */
  async crearRevendedor(revendedorData: RevendedorServex): Promise<RevendedorCreado> {
    try {
      console.log('[Servex] Creando revendedor:', revendedorData.username);
      console.log('[Servex] Datos del revendedor:', JSON.stringify(revendedorData, null, 2));
      const response = await this.client.post<ServexRevendedorResponse>('/resellers', revendedorData);
      console.log('[Servex] Respuesta de creación:', JSON.stringify(response.data, null, 2));
      
      // Si Servex no devuelve el objeto reseller, construirlo manualmente
      if (!response.data.reseller) {
        console.warn('[Servex] ⚠️ La API no devolvió el objeto reseller, construyendo manualmente');
        return {
          id: 0, // ID temporal, no lo tenemos
   ...
   */
  generarCredenciales(nombreCliente?: string): { username: string; password: string } {
    ...
    for (let i = 0; i < 3; i++) {
      letrasAleatorias += chars.charAt(Math.floor(Math.random() * chars.length));
    }
   ...
   */
  generarCredencialesRevendedor(nombreCliente?: string): { username: string; password: string; name: string } {
    const password = this.generarPasswordSeguro();
  ...
  async buscarClientePorUsername(username: string): Promise<any> {
    try {
      ...
      console.log('[Servex] Respuesta con scope default:', JSON.stringify(responseDefault.data, null, 2));
      const clientesDefault = responseDefault.data?.clients || [];
      console.log(`[Servex] Total de clientes con scope default: ${clientesDefault.length}`);
      ...
      try {
        ...
        console.log('[Servex] Respuesta con scope=todos:', JSON.stringify(responseTodos.data, null, 2));
        const clientesTodos = responseTodos.data?.clients || [];
        console.log(`[Servex] Total de clientes con scope=todos: ${clientesTodos.length}`);
        
        // Buscar en la lista con scope=todos
        const clienteEnTodos = clientesTodos.find((c: any) => c.username === username);
        if (clienteEnTodos) {
          console.log('[Servex] ✅ Cliente encontrado con scope=todos:', clienteEnTodos.username);
          return clienteEnTodos;
        }
      } catch (errorTodos: any) {
        console.warn('[Servex] ⚠️ scope=todos falló (API key no es admin):', errorTodos.message);
      }
      
      // Buscar con parámetro search en scope default
      console.log(`[Servex] Buscando con search="${username}" en scope default...`);
      const response = await this.client.get('/clients', {
        params: {
          search: username,
          limit: 100
      ...
      console.log('[Servex] Respuesta con search:', JSON.stringify(response.data, null, 2));
      const clientes = response.data?.clients || [];
      console.log(`[Servex] Encontrados ${clientes.length} clientes con búsqueda`);
      
      // Buscar coincidencia exacta
      const clienteExacto = clientes.find((c: any) => c.username === username);
      if (clienteExacto) {
        console.log('[Servex] ✅ Cliente encontrado con search:', clienteExacto.username);
        return clienteExacto;
      }
      
      console.log('[Servex] ❌ Cliente no encontrado en ningún scope');
      console.log('[Servex] 💡 Verifica: ¿Tu SERVEX_API_KEY es de Admin o Revendedor?');
      console.log('[Servex] 💡 Si es de Revendedor, solo verás clientes que ESE revendedor creó');
      return null;
    } catch (error: any) {
  ...
  async buscarRevendedorPorUsername(username: string): Promise<any> {
    try {
      ...
      // Buscar coincidencia exacta
      const revendedorExacto = revendedores.find((r: any) => r.username === username);
      if (revendedorExacto) {
        console.log('[Servex] Revendedor encontrado:', revendedorExacto.username);
  ...
  async renovarCliente(clienteId: number, dias: number): Promise<any> {
    try {
      console.log(`[Servex] Renovando cliente ID ${clienteId} por ${dias} días`);
      const response = await this.client.post(`/clients/${clienteId}/renew`, { days: dias });
  ...
  async renovarRevendedor(revendedorId: number, dias: number): Promise<any> {
    try {
      console.log(`[Servex] Renovando revendedor ID ${revendedorId} por ${dias} días`);
      const response = await this.client.post(`/resellers/${revendedorId}/renew`, { days: dias });
      console.log(`[Servex] ✅ POST /resellers/${revendedorId}/renew - ${response.status}`);
      console.log('[Servex] Respuesta de renovación:', JSON.stringify(response.data));
      return response.data;
    } catch (error: any) {
      const mensaje = error.response?.data?.message || error.response?.data?.error || error.message;
   ...
   */
  async actualizarCliente(clienteId: number, datos: Partial<ClienteServex>): Promise<any> {
    try {
      ...
      if (error.response?.data) {
        console.error('[Servex] Detalles del error:', JSON.stringify(error.response.data));
      }
   ...
   */
  async actualizarRevendedor(revendedorId: number, datos: Partial<RevendedorServex>, username?: string): Promise<any> {
    try {
      console.log(`[Servex] Actualizando revendedor ID ${revendedorId} con datos:`, JSON.stringify(datos));
      ...
      console.log('[Servex] Revendedor actual:', JSON.stringify(revendedorActual));

      // Si el revendedor no tiene categorías, usar categorías por defecto
      let categoryIds = revendedorActual.category_ids;
      if (!categoryIds || categoryIds.length === 0) {
        console.log('[Servex] ⚠️ Revendedor sin categorías, usando categorías por defecto...');
        ...
        categoryIds = defaultCategories.split(',').map(id => parseInt(id.trim()));
        console.log(`[Servex] ✅ Usando categorías por defecto: ${categoryIds.join(', ')}`);
      }
      ...
      if (password && password.length > 25) {
        console.log('[Servex] ⚠️ Password demasiado largo, truncando de', password.length, 'a 25 caracteres');
      ...
      const payload = {
        ...
        password: password,
        max_users: datos.max_users !== undefined ? datos.max_users : revendedorActual.max_users,
        account_type: datos.account_type || revendedorActual.account_type,
        ...(categoryIds && categoryIds.length > 0 && { category_ids: categoryIds }),
        ...(revendedorActual.observation && { observation: revendedorActual.observation }),
        // Si se proporciona expiration_date en datos, incluirlo (obligatorio para validity)
        ...(datos.expiration_date && { expiration_date: datos.expiration_date })
      };

      console.log('[Servex] Payload completo para actualizar:', JSON.stringify(payload));
      
      const updateResponse = await this.client.put(`/resellers/${revendedorId}`, payload);
