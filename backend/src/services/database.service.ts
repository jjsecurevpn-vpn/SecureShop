...
export class DatabaseService {
  ...
  obtenerPlanes(): Plan[] {
    const stmt = this.db.prepare('SELECT * FROM planes WHERE activo = 1 ORDER BY precio ASC');
  ...
  guardarCuentaServex(
    ...
    const stmt = this.db.prepare(`
      ...
      WHERE id = ?
    `);
    stmt.run(cuentaId, username, password, categoria, expiracion, connectionLimit, pagoId);
  }
  ...
  obtenerPlanRevendedorPorId(id: number): PlanRevendedor | null {
    const stmt = this.db.prepare('SELECT * FROM planes_revendedores WHERE id = ?');
  ...
  crearPlanRevendedor(input: CrearPlanRevendedorInput): PlanRevendedor {
    ...
    const planCreado = this.obtenerPlanRevendedorPorId(result.lastInsertRowid as number);
    if (!planCreado) {
  ...
  obtenerPagoRevendedorPorId(id: string): PagoRevendedor | null {
    const stmt = this.db.prepare('SELECT * FROM pagos_revendedores WHERE id = ?');
    const row = stmt.get(id) as PagoRevendedorRow | undefined;
    return row ? this.mapRowToPagoRevendedor(row) : null;
  }

  actualizarEstadoPagoRevendedor(id: string, estado: string, mpPaymentId?: string): void {
    const stmt = this.db.prepare(`
   ...
   */
  actualizarEstadoRenovacion(id: number, estado: string, mpPaymentId?: string): void {
    const stmt = this.db.prepare(`
      UPDATE renovaciones 
      SET estado = ?,
          mp_payment_id = ?,
